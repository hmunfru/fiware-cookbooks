...
# open INPUT tcp port 
define :open_input_tcp_port, :port => 0 do
	execute "iptables -I INPUT -p tcp -m tcp --dport #{params[:port]} -j ACCEPT" do
		not_if "iptables -S | grep -q -e 'INPUT -p tcp -m tcp --dport #{params[:port]} -j ACCEPT'"
	end
end
