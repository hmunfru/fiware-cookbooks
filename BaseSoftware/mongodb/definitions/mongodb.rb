...
define :mongodb_instance, :mongodb_type => "mongod" , :action => [:enable, :start],
    ...
    :dbpath => "/data", :configfile => "/etc/mongodb.conf", :configserver => [], :configsrv=> false, :smallfiles => false, :shardsvr => false,
  ...
  shardsvr = params[:shardsvr]
  smallfiles = params[:smallfiles]
  dbpath = params[:dbpath]
 
  configsrv = params[:configsrv]

   Chef::Log.warn('defingin')
  Chef::Log.info('in deinition')
  bind_ip = params[:bind_ip]
  port = params[:port]

  logpath = params[:logpath]
  logfile = "#{logpath}/#{name}.log"
   Chef::Log.info(logfile)
  ...
  template "#{node['mongodb']['defaults_dir']}/#{name}" do
    ...
    variables(
      ...
      "configsrv" => configsrv, #type == "configserver", this might change the port
      "shardsvr" => shardsvr,  #type == "shard", dito.
  ...
  # replicaset
  if !replicaset_name.nil? && node['mongodb']['auto_configure']['replicaset']
    if node['mongodb']['mongodb_cluster_name'] != nil
     rs_nodes = search(
      :node,
      "mongodb_cluster_name:#{replicaset['mongodb']['cluster_name']} AND recipes:mongodb\\:\\:replicaset AND mongodb_shard_name:#{replicaset['mongodb']['shard_name']}"
    ) 
   end
