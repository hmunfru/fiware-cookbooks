...
should include the `postgresqllight::server` recipe, which will include
...
`postgresqllight::server` recipe, which will include this on RHEL/Fedora
platforms.
...
`postgresqllight::config_initdb` recipe for the locale and timezone settings,
...
Credits: This `postgresqllight::config_initdb` recipe is based on algorithms
...
Credits: This `postgresqllight::config_pgtune` recipe is based on the
...
list `recipe[postgresql]` or `recipe[postgresqllight::client]`.

On systems that should be PostgreSQL servers, use
`recipe[postgresqllight::server]` on a run list. This recipe does set a
...
used. For Example:

    {
      ...
      "run_list": ["recipe[postgresqllight::server]"]
    }
