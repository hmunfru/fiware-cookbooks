...
def create_context_file
   log "el fichero #{new_resource.path}/shared/#{new_resource.application.name}.xml"
 log "application #{new_resource.application}"
 log " resource #{new_resource}"
 log " repo #{new_resource.database_master_role}" 
 ...
 log "#{host}"  
  template "#{new_resource.path}/shared/#{new_resource.application.name}.xml" do
    ...
    cookbook new_resource.context_template ? new_resource.cookbook_name.to_s : "tomcat"
    owner new_resource.owner
    group new_resource.group
    mode "644"
    variables(
      :path => "#{new_resource.application.name}",
